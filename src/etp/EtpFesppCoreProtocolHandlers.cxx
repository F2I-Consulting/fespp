...
//void setSessionToEtpHdfProxy(EtpClientSession* etpSession) {
//	COMMON_NS::EpcDocument& epcDoc = etpSession->epcDoc;
...
//			etpHdfProxy->setSession(etpSession->getIoContext(), etpSession->getHost(), etpSession->getPort(), etpSession->getTarget());
//		}
//	}
//}

void askUser(EtpClientSession* session)
{
	cout << "askUser(EtpClientSession* session)" << endl;
	std::string buffer;
...
void EtpFesppCoreProtocolHandlers::on_OpenSession(const Energistics::Etp::v12::Protocol::Core::OpenSession & os)
{
	cout << "EtpFesppCoreProtocolHandlers::on_OpenSession" << endl;
//	static_cast<EtpClientSession*>(session)->epcDoc.open("../../fakeForEtpClient.epc", COMMON_NS::EpcDocument::ETP);
//	// Ask the user about what he wants to do on another thread
//	// The main thread is on reading mode
//	std::thread askUserThread(askUser, static_cast<EtpClientSession*>(session));
